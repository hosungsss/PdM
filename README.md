...
## 3. 본 연구의 독보적 차별화 전략 (NS-PdM & MH-DCC-DNN)

기존 문헌들의 한계점을 정면 돌파하기 위해 본 연구는 다음과 같은 통합 아키텍처를 적용합니다[cite: 19, 20, 21]:

* **이중 채널 융합 (Dual-Channel):** 센서 변동량의 42%를 차지하는 사양(Spec) 차이를 반영하여, 1D-CNN 동적 채널(Within 잔차)과 사양 임베딩 정적 채널(Between 베이스라인)을 FiLM 레이어로 결합해 심슨 역설을 방지합니다[cite: 19, 21].
* **다중시간지평 이산 위험률 헤드:** 5등급 서열을 보존하면서 4개 이산 위험률($q_1 \sim q_4$)의 생존 곱($p_k = 1 - \prod (1 - q_j)$)을 통해 누적 고장확률의 구조적 단조성($0 \le p_6 \le p_{12} \le p_{24} \le p_{48} \le 1$)을 수학적으로 강제합니다[cite: 19, 20].
